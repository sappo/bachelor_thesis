...
Deutlich flexibler ist die asymmetrische Authentifizierung, denn diese ermöglicht durch den Einsatz von zwei Schlüsseln einen unsicheren Kommunikationskanal zum Austausch zu benutzen. Da der Schlüssel durch einen Man-in-the-Middle ausgetauscht worden sein könnte, wird, allein durch das Austauschen, kein Vertrauen zwischen den Kommunikationspartnern hergestellt. Die Schlüssel ausschließlich über eine zweiten sicheren Kanal zu tauschen würde den Flexibilitätsvorteil, gegenüber den symmetrischen Verfahren negieren. Die Lösung für bei der Asymmetrischen Authentifizierung heißt Zertifikate. In Abbildung~\ref{fig:design_authm2m} wird mit der Zertifikat-Autorität, dafür eine neue Instanz hinzugefügt. Diese stellt Zertifikate, nach Identitätsprüfung aus, und verifiziert die Richtigkeit des Zertifikats beim Austausch.
...
Initial muss ein neuer Marktrechner, von der Zertifikat-Autorität, ein Zertifikat beantragen. Dies kann durch den Hersteller automatisiert von Werk gesehen. Wird ein RestGateway kompromittiert, dann muss nur dieses ein neues Zertifikat bekommen. Das selbe gilt für den Mediator, da dieser keine geheimen Schlüssel anderer Benutzer aufbewahrt. Diese Architektur hat allerdings einen Single-Point of Failure. Das bedeutet, wenn die Zertifikat-Autorität nicht verfügbar ist, dann ist keine Kommunikation möglich, da Zertifikate nicht validiert werden können. Wird die Zertifikat-Autorität kompromittiert, dann benötigen alle RestGateways neue Zertifikate, was dem Worst-Case in der Symmetrischen Authentifizierung entspricht.

\section{Key Continuous Management}

Beim Key Continuous Management wird die werden die Zertifikate nach ... -> genauer nachlesen (vgl. ssh)
