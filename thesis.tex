...
Die sicher Übertragung von Daten zwischen dem FreeRADIUS-Server und dem FreeRADIUS-Client beruht einzig auf einem gut gewählten Geheimnis. Falls Geheimnisse von Werk aus aufgespielt sollen, müssen entsprechende Sicherheitsmaßnahmen getroffen werden, dass niemand außer den berechtigten Administratoren Einfluss auf die Generierung nehmen, das die Generatoren selbst sichere zufällige Ergebnisse liefern und dass das Geheimnis sicher bei einem entsprechenden FreeRADIUS-Server ankommt.

In der Beschreibung des FreeRADIUS-Server, wird von Verschlüsselung der Attribute-Value-Pairs gesprochen, nicht aber durch welchen Algorithmus. Weder die Dokumentation noch eine sonstige Quelle können genau angaben darüber treffen welche Inhalte und wie verschlüsselt werden. Diese Methode wird in der Kryptographie auch obfuscation genannt, also verschleiern. Daher sollte eine Verbindung von FreeRADIUS-Client zu FreeRADIUS-Server niemals über das Internet erfolgen. Dort ist das Risiko zu groß von einem Angreifer attackiert zu werden, der hinter die Verschleierung blicken kann.
...
Die Wartung der Benutzer über die firmeneigene Benutzerverwaltung, erleichtert die Arbeit für die Administratoren in den Fernservice-Zentralen. Einzig die Zertifikat, die für STARTTLS benötigt werden müssen in regelmäßigen Abständen gewechselt werden.
