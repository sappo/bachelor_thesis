...
Nachfolgender Abschnitt bezieht sich auf \cite{gutmann10}.
Nachdem in der Bedrohungsanalyse (Kapitel~\ref{chap:threat_analysis}) festgelegt wurde, gegen welche Bedrohungen geschützt werden soll und welche Maßnahmen dafür notwendig sind, ist der nächste Schritt die Bedrohungsmodellierung der Implementierung. Der Security Development Lifecycle (SDL) von Microsoft bietet für die Modellierung einen einfachen Prozess, welcher schnell und leicht zu verstehen ist. Anhand von Data Flow Diagrammen (DFD) wird im folgenden Kapitel modelliert. SDL entspringt keinem theoretischen akademischen Modell, es ist entstanden aus echten Problemen und gewachsen über Zeit. Zu beachten ist, dass die Bedrohungsmodellierung nur ein Teil von SDL ist.

\section{Data Flow Diagramm}

Mit Data Flow Diagrammen gehen zurück auf die 70er Jahre, sie sollen die gefährdeten Informationen der Applikation identifiziert, die Bedrohungen finden und Maßnahmen dagegen treffen. Es gibt mehrere Level von DFDs, welche den Detailgrad des Diagramms bestimmen. Für die Bedrohungsmodellierung reicht Level 0 aus. Es ist zeigt den Informationsfluss zwischen internen und externen Grenzen. Für die Bedrohungsmodellierung wird der Aufbau der Diagramme auf die fünf Objekte in Abbildung~\ref{fig:dfd_intro} beschränkt, wobei die Vertrauensgrenze in klassischen DFDs nicht vorkommt. Überschreitet eine Datenfluss eine Vertrauensgrenze können Bedrohungen diesen attackieren.
...
\section{Gefährdete Informationen}

In diesem Abschnitt werden die gefährdeten Informationen des Marktrechners mit Bezug auf die Authentifizierung und Autorisierung identifiziert. Alle Datenflüsse werden in Data Flow Diagrammen visualisiert.

\paragraph{Logdateien}

In Abschnitt~\ref{sec:analysis_measures} wurden die Maßnahmen aus der Bedrohungsanalyse festgehalten. Eine der wichtigsten ist der Logging-Mechnismus, darum müssen auch die Informationsflüsse des Logging genauer betrachtet werden. In der Abbildung~\ref{fig:dfd_logs} sehen wir, dass über eine Betrachtungsfunktion Administratoren in der Lage sind die Logs anzusehen. Logdateien können sensitive Daten enthalten, wie zum Beispiel Berechtigungsnachweise. Daher sollen nur Administratoren Zugriff auf die Daten haben.

\paragraph{HTTP-Anfragen}

Eine weitere Maßnahme sagt, dass die gesamte Kommunikation zum Marktrechner über Port 80/443 erfolgen soll. Durch ein RestGateway können HTTP-Anfragen vom Marktrechner angenommen werden. Diese werden anschließend über das LanGateway an die E*LDS Komponenten weitergegeben. Bei diesem Setup fließen die Daten durch drei Vertrauensgrenzen, welche in Abbildung~\ref{fig:dfd_http} gezeigt sind.

\begin{figure}[htbp]
\centering
\includegraphics[scale=0.4]{images/dfd_http.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=0.37]{images/dfd_http_legacy.pdf}
