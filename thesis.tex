...
In der Abbildung~\ref{fig:dfd_logs} sehen wir den Informationsfluss von Log- und Auditdaten. Ausschließlich Administratoren sollen in der Lage sein, über eine  Betrachtungsfunktion die Daten einzusehen, da dort sensitive Daten, wie etwa Passwörter, beinhaltet sein können. Unter keinen Umständen soll es möglich sein die Daten zu verändern. Denn auf diese Weise kann ein Angreifer ganz einfach seine Spuren verwischen. Für die Inhalte ist anzunehmen, dass diese nur von vertrauenswürdigen Anwendungen des Systems geschrieben werden. Die Inhalte selbst können aber von nicht vertrauenswürdigen Quellen kommen, daher muss dort mit Schadcode gerechnet werden.
...
Die Gefährdungen für Informationsflüsse können einfachheitshalber in fünf Kategorien gegliedert werden, wobei der Denial of Service nicht berücksichtigt wird, da er zu universell ist:
...
Sabotage ist ein Aspekt der bei Log- und Audit-Dateien gegeben ist, den diese enthalten Informationen, die einen Angreifer entlarven können. Im Audit könnten beispielsweise, viele Fehlgeschlagene Authentifizierungsversuche zu sehen sein, die auf einen Bruteforce-Angriff hinweisen. Sollte dieser geglückt sein und dieser Benutzer hat Rechte das Audit zu verändern, könnte der Angreifer seine Spuren verwischen. Im Log könnten beispielsweise, zahlreiche Fehlermeldungen zu sehen sein, welche ein Angreifer absichtilich produziert, um eine Schwachstelle zu finden. Durch entfernen dieser kann er sicherstellen, dass die Sicherheitslücke unentdeckt bleibt. ....  Zudem können dort bereits angesprochene sensitive Daten enthalten sein, welche einem Angreifer nicht in die Hände fallen sollen. 

\subsection{HTTP-Anfrage}

Eingehende Verbindungen sollen ausschließlich über HTTP/S erfolgen. Zu diesem Zweck gibt es das RestGateway, über welches auf den Marktrechner zugegriffen werden kann. In Abbildung~\ref{fig:dfd_http} wird der Informationsfluss über das RestGateway gezeigt. Dabei fließen die Daten durch drei Vertrauensgrenzen. Sowohl RestGateway, als auch LanGateway fungieren hierbei als Datenproxy. Deswegen muss darauf geachtet werden, dass ein Angreifer über diese kein Datenwäsche durchführen kann.
