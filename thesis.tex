...
Der Titel dieser Thesis konzentriert sich auf eine Automatisierungsanlage. Eine Automatisierungsanlage ist eine Reihe von Sensoren und Aktoren die zentral oder dezentral automatisiert gesteuert werden. In dieser Thesis wird eine Kälteanlage mit Steuerungstechnik der Firma Eckelmann betrachtet. Der Entwurf eins Sicherheitskonzeptes ist dabei Hauptbestandteil. Der Fokus liegt dabei ganz klar auf der Authentifizierung und Autorisierung. Des Weitern gibt es die Möglichkeit sich lokal oder aus der Ferne am System anzumelden. Der weitaus schwierigere Fernzugriff wird dabei in den Fokus gerückt. Der Entwurf ist die zwingende Vorarbeit für eine spätere prototypische Implementierung, unter Verwendung von existierenden Sicherheitslösungen. Abschließend wird durch eine Evaluation die prototypische Implementierung für den produktiven Einsatz bewertet.
...
Für die Authentifizierung der Clients mittels Client-Zertifikaten, benötigt es einer Certificate Authority (CA). Diese wird im Kontext des lighttpd-Server angelegt. Für den Client wird nun eine eigener CSR erzeugt. Dieser wird mit dem Zertifikat des Server und dessen privaten Schlüssel signiert, welcher anschließend zu einem X.509-Zertifikat im CRT-Format wird. Zum Schluss wird das Client-Zertifikat ins p12-Format konvertiert. Dadurch wird das Zertifikat durch ein Passwort geschützt, wodurch es für einen Angreifer, der irgendwie an dieses gelangt wertlos ist. 
