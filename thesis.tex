...
Step-By-Step Confluence-Beschreibung


\subsection{M2M-Kommunikation}

Um über die M2M-Verbindung zwischen Marktrechner und AAS Benutzer Authentifizieren zu können, benötigen wir eine Benutzerverwaltung. Das Lightweight Directory Access Protocol (LDAP) wird von den meisten Benutzerverwaltungen unterstützt und soll deshalb auch hier angewandt werden. Eine Benutzerverwaltung, die dieses Protokoll benutzt ist der ApacheDS-Server, welcher für diese Aufgabe ausgewählt wurde. LDAP legt Daten in einer Baumstruktur ab. Für die Benutzerverwaltung gibt es jedoch keine Vorgaben wie diese anzulegen ist. Jeder Knoten kann von beliebig vielen Objekt-Klassen erben. Anhand der Objekt-Klassen sind Attribute verfügbar und teilweise verpflichten. 


Der Benutzerknoten in Auflistung~\ref{lst:ldif_user} erbt von zwei Objekt-Klassen. Über \textit{person} wird verpflichten das Attribute cn verfügbar, wie weitere Attribute für Vorname, Nachname und Passwort. Der Benutzername wird über uid angegeben und spielt eine wichtige Rolle für die spätere FreeRADIUS-Konfiguration. Die Objekt-Klasse \textit{radiusProfile} können wir Rollen hinzufügen, die der FreeRADIUS-Server als radiusGroupName-Attribute kennt und auslesen kann.

\begin{lstlisting}[caption={Benutzerbeschreibung im LDIF-Format},label=lst:ldif_user]
dn: cn=Kevin Sapper+sn=Sapper,ou=users,ou=radius,dc=meinedomain,dc=local
objectClass: top
