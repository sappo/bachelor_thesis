...
Sicherheitsstandards sind von Staaten festgelegte Anforderungen an Systeme. Die Standards klassifizieren sichere Systeme anhand von Stufen, je höher die Stufe desto mehr Aufwand ist nötig, um diesen zu erreichen. Der erste Sicherheitsstandard mit hohem Verbreitungsgrad ist der Trusted Computer System Evaluation Criteria (TCSEC) und wurde 1983 vom Amerikanischen Department of Defense (DOD) veröffentlicht. Vor 1990 veröffentlichten auch andere Regierungen, unter anderen Kanada, Westdeutschland, Frankreich und Großbritannien eigene Standards. Da diese Standards nur in den jeweiligen Ländern verwendet wurden und es keine Internationale Annerkennung gab, hat die EU 1990 den Standard Information Technology Security Evaluation Criteria (ITSEC) für Europäische Staaten veröffentlicht. Dieser Standard war jedoch auf Europa begrenzt, weshalb 1996 mit den Common Criteria (CC) ein weltweit anerkannt Standard gebildet wurde. Beteiligte Staaten an den CC sind Australia/Neu Seeland, Kanada, Frankreich, Deutschland, Japan, Niederlande, Spanien, Großbritanien und die USA.

\subsubsection{Common Criteria}

Die Common Criteria sieht 7 Sicherheitsstufen für die Klassifizierung vor. Diese Evaluation Assurance Level (EAL) sind bis Stufe 4 international anerkannt. Der Aufwand ,der für höhere Stufen betrieben werden muss, ist so umfangreich, dass er nur für eine Minderheit an Unternehmen zutrifft. Die aktuelle Version der CC ist 3.1. Release 4 und wurde im September 2012 veröffentlicht. Alle weiteren Erläuterungen beziehen sich auf diese Version und sind aus den Quellen \cite{ccp1, ccp2, ccp3, bsi_ccguide} entnommen.
...
Die Common Criteria sieht zwei Möglichen Vorgehen vor. Zum einen kundengetrieben zum anderen entwicklergetrieben. Bei der ersten Möglichkeit stellt der Kunde seine Anforderungen an Hardware und Software in einem Protection Profile (PP) zusammen. Anhand dessen kann er in sein gewünschtes System/Produkt entwickeln lassen. Die Entwickler müssen mit dem Security Target (ST) nachweisen, dass das entwickelte System/Produkt den Anforderungen aus dem Protection Profile genügt. Mit Hilfe von PP und ST kann von einer unabhängigen Prüfstelle evaluiert und zertifiziert werden, ob die Informationen in der Dokumenten der Wahrheit entsprechen.
...
Die Inhalte aus PP und ST sind zunächst identisch, wobei das ST explizit auf die Umsetzung eingeht. Da eine Umsetzung nicher zweiten Möglichkeit fällt der Kunden weg und die Entwickler schreiben ein Security Target, um von einer Prüfstelle ein Zertifikat zu erhalten, welches als Qualitätsmerkmal ihres Produktes beworben werden kann.
t im Rahmen dieser Thesis ist, wird das ST nicht weiter beschrieben.

\paragraph{Protection Profile}

Die Erstellung eines Protection Profiles, wird im Folgenden mit der Erklärungs-Methode beschrieben. Diese besteht aus insgesamt sieben Teilen. Die Informationen darüber stammen aus \cite{bsi_ccguide}. Als erstes werden Konformitätsansprüche beschrieben. Darunter fallen Verweise auf die Version des CC, abhängige Protection Profile und welche EAL-Pakete genutzt werden. Abschließend wird noch beschrieben, wie strikt sich PPs und STs and dieses PP halten müssen. Im zweiten Schritt werden die Sicherheitsprobleme definiert. Dabei werden in Bezug auf das zu evaluierende Ziel (Target of Evaluation--TOE) Gesetze, sonstige Regulierungen und Bedrohungen analysiert. Bei den Bedrohungen liegt der Fokus auf den zu schützenden Gütern. Im dritten Schritt werden zu den Sicherheitsproblemen aus den vorherigen Schritt eine abstrakte, präzise Lösungsvorschläge gemacht. Dabei soll sich auf das Was konzentriert werden und nicht beschrieben werden Wie etwas zu lösen ist. Im CC-Kontext hießen diese Lösungsvorschläge Secrurity Objects. Im vierten Schritt werden aus den Security-Objects detailliertere Security Functional Requirements (SFRs). Die SFRs werden dabei an relevante, thematische Gruppen gekoppelt, welche im CC Standard vorgegeben sind. Die Beschreibung als SFR umfasst die Beteiligten, die Informationsflüsse, die OperationenEin Protection Profile besteht aus sechs Hauptteilen die im Folgenden beschrieben werden. Die Informationen darüber stammen aus \cite{bsi_ccguide}. Als erstes werden Konformitätsansprüche beschrieben. Darunter fallen Verweise auf die Version des CC, abhängige Protection Profile und welche EAL-Pakete genutzt werden. Abschließend wird noch beschrieben, wie strikt sich PPs und STs and dieses PP halten müssen. Im zweiten Schritt werden die Sicherheitsprobleme definiert. Dabei werden in Bezug auf das zu evaluierende Ziel (Target of Evaluation--TOE) Gesetze, sonstige Regulierungen und Bedrohungen analysiert. Bei den Bedrohungen liegt der Fokus auf den zu schützenden Gütern. Im dritten Schritt werden zu den Sicherheitsproblemen aus den vorherigen Schritt eine abstrakte, präzise Lösungsvorschläge gemacht. Dabei soll sich auf das Was konzentriert werden und nicht beschrieben werden Wie etwas zu lösen ist. Im CC-Kontext hießen diese Lösungsvorschläge Secrurity Objects. Im vierten Schritt werden aus den Security-Objects detailliertere Security Functional Requirements (SFRs). Die SFRs werden dabei an relevante, thematische Gruppen gekoppelt, welche im CC Standard vorgegeben sind. Die Beschreibung als SFR umfasst die Beteiligten, dei Informationsflüsse, die Operationen und die Daten. Damit sind die Sicherheitsanforderungen an das System/Produkt festgelegt. In Schritt fünf wird abschließend festgelegt, wie das TOE zu evaluieren ist. Im siebten und letzten Schritt wird dann eine Einleitung in Prosa geschrieben.
...
\section{Methoden zur Bedrohungsanalyse}
